...
public class Encoder {
		...
		try {
			...
			while (true) {
				...
				if (step == 0) {
					...
					if (m.matches()) {
						//System.out.println("m.matches for duration");
	...
	public void encode(File source, File target, EncodingAttributes attributes,
		...
		try {
			...
			if (lastWarning != null) {
				if (!SUCCESS_PATTERN.matcher(lastWarning).matches() && (lastWarning.indexOf("muxing overhead") == -1)) {
